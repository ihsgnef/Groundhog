...
def construct_vocabulary(dataset, oov_rate, level):
    ...
    return dict(zip(words, range(up_to))), [x[1]/all_freq for x in freqs]
...
def main(parser):
    ...
    vocab, freqs = construct_vocabulary(dataset, o.oov_rate, o.level)
    ...
    keys = {'oov': oov_default, 'freqs': numpy.array(freqs), 'vocabulary': vocab}
    all_keys = dict(keys.items() + data.items())
    
    numpy.savez(o.dest, **all_keys)
    inv_map = [None] * (len(vocab) + 1)
